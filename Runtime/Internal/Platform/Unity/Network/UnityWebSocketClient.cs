...
namespace Gamania.GIMChat.Internal.Platform.Unity.Network
{
    ...
    public class UnityWebSocketClient : IWebSocketClient
    {
        ...
        public event Action<GimException> OnError;
        
        public GimConnectionState State => _state;
        public bool IsConnected => _state.IsConnected();
        public WebSocketConfig Config => _config;

        private WebSocket _webSocket;
        private GimConnectionState _state = GimConnectionState.Closed;
        ...
        public void Connect(WebSocketConfig config)
        {
            if (config == null)
            {
                OnError?.Invoke(new GimException(GimErrorCode.InvalidParameter, "WebSocketConfig cannot be null"));
                return;
            }

            _config = config;
            TransitionToState(GimConnectionState.Connecting, "Connect() called");
            ...
            {
                OnError?.Invoke(new GimException(GimErrorCode.InvalidParameter, $"Failed to build URL: {ex.Message}", ex));
                return;
            }
            ...
            catch (Exception ex)
            {
                ...
                OnError?.Invoke(new GimException(GimErrorCode.WebSocketConnectionFailed, $"Connect failed: {ex.Message}", ex));
            }
        }
        ...
        public void Disconnect()
        {
            if (_webSocket != null)
            {
                ...
                if (_state == GimConnectionState.Open)
                {
                    TransitionToState(GimConnectionState.Closing, "Disconnect() called");
                }
                ...
                catch (Exception ex)
                {
                    ...
                    TransitionToState(GimConnectionState.Closed, "Disconnect exception");
                }
            }
        }
        ...
        public Task SendAsync(string message)
        {
            ...
            if (!SendRaw(message))
            {
                throw new GimException(GimErrorCode.WebSocketConnectionFailed, "Failed to send WebSocket message.");
            }
        ...
        private void HandleOnOpen()
        {
            ...
            TransitionToState(GimConnectionState.Open, "WebSocket Open");
        ...
        private void HandleOnClose(WebSocketCloseCode closeCode)
        {
            ...
            TransitionToState(GimConnectionState.Closed, reason);
            ...
            MainThreadDispatcher.Enqueue(() =>
            {
                ...
                else
                {
                    ...
                    var errorCode = closeCode == WebSocketCloseCode.Abnormal
                        ...
                        : GimErrorCode.WebSocketConnectionClosed;
                    OnError?.Invoke(new GimException(errorCode, $"WebSocket closed with code: {(ushort)closeCode}"));
                }
            });
        }
        ...
        private void HandleOnMessage(byte[] data)
        {
            ...
            catch (Exception ex)
            {
                ...
                MainThreadDispatcher.Enqueue(() =>
                {
                    OnError?.Invoke(new GimException(GimErrorCode.MalformedData, $"Failed to decode message: {ex.Message}", ex));
                });
            }
        }
        ...
        private void HandleOnError(string errorMessage)
        {
            ...
            if (_state == GimConnectionState.Connecting)
            {
                TransitionToState(GimConnectionState.Closed, "Connection error");
            }
            else if (_state == GimConnectionState.Open)
            {
                TransitionToState(GimConnectionState.Closed, "Network error");
            }

            MainThreadDispatcher.Enqueue(() =>
            {
                OnError?.Invoke(new GimException(GimErrorCode.WebSocketConnectionFailed, errorMessage));
            });
        }

        private void TransitionToState(GimConnectionState newState, string reason)
        {
            if (_state == newState)
                return;
        ...
        private bool SendRaw(string message)
        {
            if (!_state.IsConnected() || _webSocket == null || _webSocket.State != WebSocketState.Open)
            {
                ...
                MainThreadDispatcher.Enqueue(() =>
                {
                    OnError?.Invoke(new GimException(GimErrorCode.ConnectionRequired, error));
                });
                return false;
            }
            ...
            catch (Exception ex)
            {
                ...
                MainThreadDispatcher.Enqueue(() =>
                {
                    OnError?.Invoke(new GimException(GimErrorCode.WebSocketConnectionFailed, $"Send failed: {ex.Message}", ex));
                });
                return false;
            }
        }
